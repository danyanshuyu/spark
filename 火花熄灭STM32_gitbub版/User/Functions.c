...
void Screen_update()                                                           //屏幕数据更新显示程序--事件开始时间
{
	  //复位各显示相关标志位
      Menu_FlagStatus = 0;          //菜单键取值状态，取值0/1/2/3，对应4个菜单
	  System_FlagStatus = -1;		//系统一级菜单的取值，0~7，-1为初始化值
	  LR_FlagStatus = 0;            //左右键计数状态，按一次右键加1，按一次左键减1
	  UD_FlagStatus = 0;            //上下键计数状态，按一次下键加1，按一次上键减1
	  Button_Dn = 2;			    //警报界面时，上下键按一次更新显示另外2条信息
	  
	  Alarm_num = _24C08_I2C_HalfwordRead(0);								   //最新火花事件数
	  Alarm_End_num = _24C08_I2C_HalfwordRead(34);							   //最新火花结束事件数
	  ...
	  itoaT(Alarm_num,JINGBAO_CUT);											   
	  SetTextView(1,38,JINGBAO_CUT);										   //火花事件总数显示
...
void Screen_End_update()                                                           //屏幕数据更新显示程序--事件结束时间
{
	  //复位各显示相关标志位
      Menu_FlagStatus = 0;          //菜单键取值状态，取值0/1/2/3，对应4个菜单
	  System_FlagStatus = -1;		//系统一级菜单的取值，0~7，-1为初始化值
...
void Fault_Display_A(u16 num)					                               //故障显示A区域		num-第几个故障
{
        if(num <= 0)
		{
			;
...
void Disable_Display_A(u16 num)					                               //关闭显示A区域		num-第几个关闭事件
{
        if(num <= 0)
		{
			;
...
void Alarm_Display_A(u16 num)					                               //警报显示A区域		num-第几次报警
{
        if(num <= 0)
		{
			;
...
void Alarm_Display_B(u16 num)					                               //警报显示B区域		num-第几次报警
{
        if(num <= 0)
		{
			;
...
void Alarm_End_Display_A(u16 num)					                               //警报结束显示A区域		num-第几次报警
{
        if(num <= 0)
		{
			;
...
void Alarm_End_Display_B(u16 num)					                               //警报结束显示B区域		num-第几次报警
{
        if(num <= 0)
		{
			;
